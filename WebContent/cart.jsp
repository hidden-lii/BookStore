...
<!--头部搜索开始-->
	<div class="header">
		...
		<div class="search fl">
			...
			<div class="s_hot hui">热门搜索：武侠 </div>
		</div>
	</div>
<!--头部搜索结束-->
<!--菜单开始-->
	...
	<div class="main mt10">
		...
		<!--购物车开始-->
			...
			<s:iterator id="cartItem" value="#session.cart">
				...
				<div class="car_c fl">
					<p class="bh fl"><s:property value="value.bi.id"/> </p>
					<p class="spmc fl"><s:property value="value.bi.name"/></p>
					<p class="dj fl">￥<s:property value="value.bi.price"/></p>
					<p class="sl fl">
						<input type="text" value="${value.quantity }" size="10"
						 onchange="window.location='updateSelectedQuantity?bookInfoId=${value.bi.id}&quantity='+this.value;" />				
					</p>
					<p class="je fl">￥<s:property value="value.quantity*value.bi.price"/></p>
					<p class="del fl"><a href="deleteSelectedOrders?bookInfoId=${value.bi.id }">
									<img src="images/d013.jpg"/></a>
					</p>
				</div>
				<s:set var="sumPrice" value="#sumPrice+value.quantity*value.bi.price" />
			</s:iterator>
